...
#define TESTBEGIN_EX(Name, List)                                                \
    class Test##Name : public UnitTest::Test                               \
    {                                                                      \
    public:                                                                \
		Test##Name() : Test(#Name, UnitTestSuite::GetSuiteName(), __FILE__, __LINE__, true) {}  \
    private:                                                               \
        virtual void RunImpl();   \
...
#define TESTBEGIN(Name) TESTBEGIN_EX(Name, UnitTest::Test::GetTestList())
#define TESTCONTINUE(Name) bool Test##Name::RunContinueImpl() const
...
#define TEST_FIXTURE_BEGIN_EX(Fixture, Name, List)                                         \
    class Fixture##Name##Helper : public Fixture									 \
	{																				 \
	public:																			 \
        explicit Fixture##Name##Helper(UnitTest::TestDetails const& details) : m_details(details){} \
        void RunImpl();                           \
		bool RunContinueImpl();   \
        UnitTest::TestDetails const & m_details;                                      \
    private:                                                                         \
        Fixture##Name##Helper(Fixture##Name##Helper const&);                         \
        Fixture##Name##Helper& operator =(Fixture##Name##Helper const&);             \
    ...
    {                                                                                \
    public:                                                                          \
	    Test##Fixture##Name() : Test(#Name, UnitTestSuite::GetSuiteName(), __FILE__, __LINE__, true), fixtureHelper(m_details) {} \
    private:                                                                         \
        virtual void RunImpl();             \
		virtual bool RunContinueImpl();   \
			...
			{																		 \
				UnitTest::CurrentTest::Results()->OnTestFailure(UnitTest::TestDetails(m_details, __LINE__),   \
					"Unhandled exception while constructing fixture " #Fixture);         \
																					 ...
																					 \
    bool Test##Fixture##Name::RunContinueImpl()  \
	{																				 \
		bool ctorOk = false;														 \
		try {																		 \
			ctorOk = true;															 \
			return UnitTest::ContinueTest(fixtureHelper, m_details);					 \
		}																			 \
		catch (UnitTest::AssertException const& e)											 \
		{																			 \
			UnitTest::CurrentTest::Results()->OnTestFailure(UnitTest::TestDetails(m_details.testName, m_details.suiteName, e.Filename(), e.LineNumber()), e.what()); \
...
#define TEST_FIXTURE_BEGIN(Fixture,Name) TEST_FIXTURE_BEGIN_EX(Fixture, Name, UnitTest::Test::GetTestList())
#define TEST_FIXTURE_CONTINUE(Fixture,Name) bool Fixture##Name##Helper::RunContinueImpl()
