...
The Python interface to InductorHtn is a single class called HtnPlanner with a few simple methods. There are only two files you need to use it: 
...
~~~
# See GettingStarted.md for a whole bunch of pointers to understand HTNs, Prolog, etc. This file just describes how to use the framework
...
# Traces of what is happening are sent to the system debug output stream which can be seen on windows with https://docs.microsoft.com/en-us/sysinternals/downloads/debugview
# These traces are much like the standard Prolog traces and will help you understand how the queries and HTN tasks are running and what path they are taking
...
# Gets all possible plans that are generated by a query (will return a list of plans, each which is a list of terms)
...
# Terms are just dictionaries with one key, the name of the term, and one value: a list of more terms
